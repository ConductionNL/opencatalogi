...
- **Acceptance criteria**: Requirements decomposed respecting the
  consume-vs-build split; counter write path (in-request deferred vs queued
  flush) and stats-visibility role (catalog editors + admins proposed)
...
- **Status**: todo
- **Acceptance criteria**:
  - Test proving counter objects and counting-path logs contain no
    request-derived attributes beyond (publication, date, kind, count).
  - README/docs: what is counted, what is deliberately not (requests ≠
    visitors; no personal data), counting-start caveat for officers.
