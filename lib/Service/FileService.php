...
class FileService
{
     ...
     *
     * @throws Exception When share creation fails.
     */
     ...
     * @param integer|null $shareType   Share type: 0=user, 1=group, 3=public link, 4=email, 6=federated, 7=circle, 10=Talk.
     * @param integer|null $permissions Permissions: 1=read, 2=update, 4=create, 8=delete, 16=share, 31=all (default: 31, public: 1).
     *
    ...
    public function createShareLink(string $path, ?int $shareType = 3, ?int $permissions = null): string
    {
        ...
        try {
            // Note: to create share links for folders, remove this try-catch and use only setTarget (not setNodeId).
            $file = $userFolder->get(path: $path);
        } catch (NotFoundException $e) {
     ...
     *
     * @return JSONResponse|string An error response if creating the file in NextCloud failed,
     ...
     *
     * @param string $folderPath Path (from root) to where you want to create a folder, include the name of the folder (e.g. /Media/exampleFolder).
     *
    ...
    public function createFolder(string $folderPath): bool
    {
        ...
        $userFolder = $this->rootFolder->getUserFolder(userId: $currentUserId);
     ...
     * @param string $filePath Path (from root) where to save the file, including name and extension (e.g. example.pdf).
     *
    ...
    public function uploadFile(mixed $content, string $filePath): bool
    {
        ...
        $currentUser = $this->userSession->getUser();
        if ($currentUser !== null) {
            $uploadUserId = $currentUser->getUID();
        } else {
            $uploadUserId = 'Guest';
        }

        $userFolder  = $this->rootFolder->getUserFolder(userId: $uploadUserId);
     ...
     * @param mixed   $content   The content of the file.
     * @param string  $filePath  Path (from root) where to save the file, including name and extension (e.g. example.pdf).
     * @param boolean $createNew If set to true this function will create a new file if it doesn't exist yet (default false).
     *
     * @return boolean True if successful.
     *
     * @throws Exception In case we can't write to file because it is not permitted.
    ...
    public function updateFile(mixed $content, string $filePath, bool $createNew = false): bool
    {
        ...
        $currentUser = $this->userSession->getUser();
        if ($currentUser !== null) {
            $updateUserId = $currentUser->getUID();
        } else {
            $updateUserId = 'Guest';
        }

        $userFolder  = $this->rootFolder->getUserFolder(userId: $updateUserId);
    ...
    public function deleteFile(string $filePath): bool
    {
        ...
        $currentUser = $this->userSession->getUser();
        if ($currentUser !== null) {
            $deleteUserId = $currentUser->getUID();
        } else {
            $deleteUserId = 'Guest';
        }

        $userFolder  = $this->rootFolder->getUserFolder(userId: $deleteUserId);
     ...
     *
     * @return Mpdf A Mpdf object. Use Output(name: $filename, dest: Destination::FILE) to create the actual file.
     *             Use "rmdir(directory: '/tmp/mpdf');" after this to clean up temporary files.
     *
     * @throws MpdfException|LoaderError|RuntimeError|SyntaxError When PDF creation fails.
     */
