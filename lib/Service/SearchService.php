...
class SearchService
{
    ...
    /**
     * Base object configuration for database queries.
     *
     * @var array
     */
     ...
     *
     * @param ElasticSearchService $elasticService   The service responsible for handling interactions
     ...
     * @param array $newAggregation      The new aggregation data to merge in.
     *
     ...
     *
     * @param array|null $existingAggregations The current aggregations to merge into.
     * @param array|null $newAggregations      The new aggregations to merge from.
     *
     ...
     *
     * @param array $a First result array to compare.
     * @param array $b Second result array to compare.
     *
     * @return integer Negative, zero, or positive comparison result.
     */
    ...
    public function search(array $parameters, array $elasticConfig, array $dbConfig, array $catalogi = []): array
    {
        ...
        $promises        = [];
        $localSearchUrl  = $this->urlGenerator->getAbsoluteURL(
        ...
        foreach ($directory as $instance) {
            ...
            if (isset($parameters['_catalogi']) === true
                && in_array($instance['catalog'], $parameters['_catalogi'], true) === false
            ) {
                continue;
            }

            if ($instance['search'] === $localSearchUrl) {
                continue;
            }
     ...
     *
     * @return array The updated filters array.
     */
     ...
     *
     * @return array The search conditions array.
     */
     ...
     * @param array $filters Query parameters from request.
     *
     * @return array The cleaned filters array.
     */
     ...
     * @param array $filters Query parameters from request.
     *
     * @return array The search parameters array.
     */
     ...
     * Also checks for queryParams ending on [] like: ?queryParam[$nameKey][] (or just ?queryParam[]), if this is the
     * case this function will add given value to an array of [queryParam][$nameKey][] = $value or
     * [queryParam][] = $value. If none of the above this function will just add [queryParam] = $value to $vars.
     *
