...
- [x] Bundle two value lists (HVD categories → authority URIs; EU MDR
...
- [x] Extend the `x-dcat` annotation contract (DCAT-004) with an optional `hvd`
...
- [x] `DcatMappingService` / `DcatService`: render `dcatap:hvdCategory` +
...
- [x] Instance-level source metadata: ensure `GET /api/dcat`'s `dcat:Catalog`
...
- [x] Admin settings: display the canonical harvest-source URL and add the
  ...
  - Acceptance: URL shown; validation lists violations; feed still served
    (validation advisory)
- [x] Newman: assert instance source metadata, HVD triples on an HVD-declared
