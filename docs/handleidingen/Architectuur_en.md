...
## Under the Hood
Open Catalogue actually consists of a couple of technical components working together. For a start, it consists of several objects (Catalogi, Publications, Documents, and Index) which are stored in OpenRegister (object store). Publications give a basic workflow management setup. The Open Catalogue search endpoint uses OpenRegister's search capabilities for local queries and combines them with results from remote catalogs via federated search.
