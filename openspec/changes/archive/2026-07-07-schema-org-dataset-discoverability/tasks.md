...
- [x] Add a per-catalog/per-schema `schema:Dataset` election flag to the catalog
...
- [x] Add a schema.org serializer that maps an OR object + its schema
...
- [x] Content-negotiate `application/ld+json` (and `?format=schema`/`.jsonld`)
...
- [x] Catalog representation lists publicly visible publications as
...
- [x] Newman: assert marker-typed node, elected `Dataset` shape (name +
