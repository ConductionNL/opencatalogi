...
if [ -n "$EXISTING_CATALOG" ]; then
    ...
    # Build the JSON body via env-vars so Python does the escaping — not
    # the shell. Interpolating shell vars directly into a `python3 -c ...`
    # heredoc breaks on any single quote or `'''` in the value.
    catalog_body=$(PUB_REGISTER="$PUB_REGISTER" PUB_SCHEMA="$PUB_SCHEMA" DOC_SCHEMA="$DOC_SCHEMA" \
        python3 -c "
import json, os
print(json.dumps({
    'slug':        'publications',
    ...
    'registers':   [int(os.environ['PUB_REGISTER'])],
    'schemas':     [int(os.environ['PUB_SCHEMA']), int(os.environ['DOC_SCHEMA'])],
    'listed':      True,
    'published':   '2026-01-01T00:00:00+00:00',
    '@self':       {'slug': 'publications', 'published': '2026-01-01T00:00:00+00:00'},
}))
")
    CATALOG_ID=$(json_post "$OR_API/objects/$PUB_REGISTER/$CAT_SCHEMA" "$catalog_body" \
...
#
# seed_pub() is idempotent: it looks up an existing publication by @self.slug
# first and reuses it if found. Re-running the script therefore no longer
# creates duplicate publications.
...
" 2>/dev/null || true)
    ...
    # Build the JSON body via env-vars — see catalog step for rationale.
    local body
    body=$(SLUG="$slug" TITLE="$title" SUMMARY="$summary" DESCRIPTION="$description" \
        python3 -c "
import json, os
print(json.dumps({
...
# seed_doc() is idempotent: it looks up an existing document by title (there
# is no slug on documents) and reuses it if found. Re-running the script
# therefore no longer creates duplicate documents.
...
try:
    d = json.load(sys.stdin)
    for r in d.get('results', []):
        if r.get('title') == os.environ['TITLE']:
            print(r['@self']['id'])
            break
except: pass
" 2>/dev/null || true)
    ...
    # Build the JSON body via env-vars — see catalog step for rationale.
    local body
    body=$(TITLE="$title" FILENAME="$filename" SUMMARY="$summary" \
        PUB_ID="$pub_id" PUB_SLUG_ARG="$pub_slug" PUB_TITLE_ARG="$pub_title" \
        python3 -c "
import json, os
print(json.dumps({
    'title':           os.environ['TITLE'],
    ...
    'publicatiedatum': '2026-05-01T00:00:00+00:00',
    'publication': {
        'id':    os.environ['PUB_ID'],
        'slug':  os.environ['PUB_SLUG_ARG'],
        'title': os.environ['PUB_TITLE_ARG'],
    },
}))
")
...
# PDF text-literals must escape \\, (, ) — otherwise `(text) Tj` breaks
# the content stream for any caller that passes those characters.
...
upload_pdf() {
    ...
    local body
    body=$(FILENAME="$filename" B64="$b64" python3 -c "
import json, os
print(json.dumps({
...
PUB_TOTAL=$(curl -s "${AUTH[@]}" "$OC_API/publications?_search=convenant&_limit=10" | python3 -c "
import sys, json
...
SEARCH_RESULTS=$(curl -s "${AUTH[@]}" "$OC_API/search?_search=convenant&_limit=10")
