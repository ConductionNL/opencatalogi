...
#### Scenario: REQ-STD-001 — Metrics endpoint returns valid Prometheus format
...
#### Scenario: REQ-STD-002 — App info metric is present
...
#### Scenario: REQ-STD-003 — App up metric indicates health
...
#### Scenario: REQ-STD-004 — Metrics endpoint requires admin authentication
...
#### Scenario: REQ-STD-005 — Metrics endpoint handles database errors gracefully
...
#### Scenario: REQ-PUB-001 — Publication counts by status and catalog
...
#### Scenario: REQ-CAT-001 — Catalog count is accurate
- GIVEN 5 catalogs exist in the system
- WHEN the metrics endpoint is called
- THEN `opencatalogi_catalogs_total` gauge MUST have value 5

#### Scenario: REQ-CAT-002 — Catalog count after creation and deletion
...
#### Scenario: REQ-CAT-003 — Catalog count uses pattern matching on schema title
...
#### Scenario: REQ-LST-001 — Listing counts by status
...
#### Scenario: REQ-SRH-001 — Search request count
...
#### Scenario: REQ-SRH-002 — Search metrics table does not exist
...
#### Scenario: REQ-SRH-003 — Search count reflects cumulative total
...
#### Scenario: REQ-FED-001 — Directory entry count
...
#### Scenario: REQ-FED-002 — Federation reachability
...
#### Scenario: REQ-FED-003 — No federation configured
...
#### Scenario: REQ-HLT-001 — All checks pass
...
#### Scenario: REQ-HLT-002 — Database check fails
...
#### Scenario: REQ-HLT-003 — Filesystem check fails
...
#### Scenario: REQ-HLT-004 — Health check includes search backend status
...
#### Scenario: REQ-HLT-005 — Health check response time
...
#### Scenario: REQ-VER-001 — Nextcloud version label on info metric
...
#### Scenario: REQ-VER-002 — PHP version label on info metric
- GIVEN the PHP version is 8.2.15
- WHEN the metrics endpoint is called
- THEN `opencatalogi_info` MUST include a `php_version` label with value "8.2.15"

#### Scenario: REQ-VER-003 — Version information is always available
...
#### Scenario: REQ-PRF-001 — Metrics response time under normal load
...
#### Scenario: REQ-PRF-002 — Metrics queries use efficient aggregation
...
#### Scenario: REQ-PRF-003 — Metrics endpoint does not cache stale data
...
#### Scenario: REQ-NMC-001 — Metric names use app prefix
...
#### Scenario: REQ-NMC-002 — Counter metrics have _total suffix
...
#### Scenario: REQ-NMC-003 — Histogram metrics have _seconds suffix for duration
...
#### Scenario: REQ-NMC-004 — Gauge metrics represent current state
...
- Nextcloud IConfig for Nextcloud server version
