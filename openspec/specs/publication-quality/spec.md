# publication-quality Specification

## Purpose

@e2e exclude pure backend/API spec — every scenario asserts server-side metadata-quality scoring over the rendered `dcat:Dataset` (MQA/FAIR sub-scores, the weighted total, the missing-property breakdown), optional W3C DQV exposure in the DCAT feed, and the authenticated per-catalog roll-up endpoint; the publisher dashboard reuses the existing `publication-usage-analytics` widget surface and has no new OpenCatalogi-owned browser view. Covered by unit + Newman API tests.

OpenCatalogi derives a metadata-quality (MQA/FAIR) score for each publication from the same `dcat:Dataset` node the DCAT layer already renders (`dcat-ap-harvest`) — no second mapping, no new authoritative field, and no new visibility rule. The score is advisory (it never gates publishing, depublishing, or retention, and never alters the OR `publicatiedatum <= now` visibility) and privacy-safe (computed over catalog metadata only, no reader/request/PII data). It closes the loop that being *harvested* opens: it lets publishers see why their datasets rank poorly in data.overheid.nl / the EU MQA and what to fix.
