...
- [x] Add admin-config: MQA dimension weights (default EU MQA weights) and a
...
- [x] Add a `QualityService` that consumes the rendered `dcat:Dataset` for a
...
- [x] Extend the DCAT serializer to emit `dqv:hasQualityMeasurement` nodes when a
...
- [x] Add the per-catalog quality roll-up on the authenticated stats API
...
- [x] Persist roll-ups (if needed) as derived `qualityMetric` OR objects
...
- [x] Newman: assert high vs. penalised scores, DQV on/off, catalog roll-up, and
