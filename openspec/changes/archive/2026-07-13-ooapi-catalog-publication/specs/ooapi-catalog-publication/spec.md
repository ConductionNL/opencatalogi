...
@e2e exclude pure backend/API spec — every scenario asserts a materialized-object
read path, schema-driven JSON mapping, pagination, and a consumer-credential
authenticated public HTTP surface with no browser-observable UI surface (the
per-catalog admin toggle/credential surface is part of the existing
admin-settings UI spec); covered by Newman API tests and PHPUnit instead.

Expose OpenCatalogi catalogs as machine-readable **OOAPI 5.0** (Open Onderwijs
API) course-catalog resources — `organizations`, `programs`, `courses`,
`offerings` — so SURFeduhub and institutional consumers can read an
institution's course catalog. This is the OpenCatalogi third of a three-repo
contract: scholiq (already merged, `delegate-ooapi-to-opencatalogi`) supplies
source objects and a `DataExchangeJob` (`target: ooapi-catalog`); OpenConnector
(sibling change, not built here) materializes them as OpenCatalogi-owned OR
objects via its existing `Synchronization` target-write path; this spec
defines the resulting register/schema shape and the read-only, schema-driven
rendering layer over it (hydra ADR-022 — no bespoke storage beyond the
OpenCatalogi-owned schemas themselves, no bespoke query layer).

## Context

**Relation to existing specs:**

- `dcat-ap-harvest` / `structured-data-discoverability`: the precedent this
  spec mirrors — schema-driven mapping annotation (`x-dcat`/`x-schema-org` →
  this spec's `x-ooapi`), read-only rendering over `searchObjects`, per-catalog
  enable toggle, federation-directory advertisement. The divergence (design.md
  D1) is that `course`/`program`/`offering` have no pre-existing OpenCatalogi
  object to render — they are materialized by OpenConnector first.
- `publications` (PUB-001..015) / `catalogs` (CAT-001..016): supply the
  catalog-scoping and OR object-search query pattern reused here; `organization`
  additionally reuses the existing Organisation object (PUB-CON-001).
- `federation` (FED-009, FED-OR-001/002): the directory/listing machinery is
  extended with an `ooapiEndpoint` field (mirrors `dcatEndpoint`); broadcast/
  retry behaviour is unchanged.
- `admin-settings`: hosts the per-catalog `hasOoapi` toggle, consumer-credential
  management, and the feed-validation action; every new config key this spec
  introduces must appear in that spec's inventory table (per
  `opencatalogi-adopt-or-abstractions`).
- `opencatalogi-adopt-or-abstractions`: controllers resolve register/schema
  pairs via `RegisterResolverService`; no raw SQL against OR's magic tables.
- `cross-origin-api-access`: CORS posture for the new public endpoints.
- `scholiq/openspec/specs/course-management/spec.md` ("Publish course catalog
  via OOAPI 5.0") and `scholiq/openspec/specs/data-exchange/spec.md`
  ("Delegate wire protocols to OpenConnector", `target: ooapi-catalog"): the
  upstream contract this spec is the read-side consumer of.
