...
class DemoDataServiceTest extends TestCase {
	...
	protected function setUp(): void {
		$this->appDir = sys_get_temp_dir() . '/oc-demo-' . uniqid();
	...
	protected function tearDown(): void {
		$file = $this->appDir . '/lib/Settings/opencatalogi_mock_register.json';
	...
	private function shipDescriptor(int $objects = 2): void {
		file_put_contents(
			$this->appDir . '/lib/Settings/opencatalogi_mock_register.json',
			json_encode(
				[
					'x-openregister' => ['type' => 'mock', 'app' => 'opencatalogi'],
					'components' => [
						'registers' => ['opencatalogi' => ['schemas' => ['Publication']]],
						'schemas' => ['Publication' => ['type' => 'object']],
						'objects' => array_fill(0, $objects, ['@self' => ['register' => 'opencatalogi', 'schema' => 'Publication']]),
					],
				]
			)
		);
	...
	private function importerSpy(): object {
		return new class {
			...
			public function importFromApp(string $appId, array $data, string $version, bool $force): array {
				...
				return ['registers' => ['opencatalogi'], 'schemas' => ['Publication']];
			}
		};
	}
	...
	public function testAMalformedDescriptorOffersOnlyDeclining(): void {
		file_put_contents($this->appDir . '/lib/Settings/opencatalogi_mock_register.json', 'not json at all');
	...
	/**
	 * A descriptor with no objects block is still a real dataset, with nothing
	 * to count.
	 *
	 * @return void
	 */
	public function testADescriptorWithNoObjectsBlockOffersTheSetWithNoCount(): void {
		file_put_contents($this->appDir . '/lib/Settings/opencatalogi_mock_register.json', json_encode(['components' => ['schemas' => []]]));
	...
	public function testItImportsUnderItsOwnConfigurationIdentity(): void {
		...
		$this->assertSame('opencatalogi.demo', $spy->seen['appId']);
	}
	...
	public function testUnparsableJsonThrowsRatherThanImportingNothing(): void {
		file_put_contents($this->appDir . '/lib/Settings/opencatalogi_mock_register.json', 'not json');
		$this->container->expects($this->never())->method('get');
