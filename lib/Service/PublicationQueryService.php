...
class PublicationQueryService
{
     ...
     * @param IAppConfig|null      $config      App config; the catalog-enumeration path uses it to locate the catalog
     *                                          register/schema.
    ...
    public function assemblePublicSearchResults(array $queryParams, object $objectService): array
    {
        ...
        if (empty($scope['schemas']) === true) {
            ...
            $this->logger?->warning(
                'PublicationQueryService::assemblePublicSearchResults: empty envelope, no schemas in catalog scope',
                [
        ...
        // `publicatiedatum <= now AND (depublicatiedatum >= now OR $exists false)`).
        ...
        // Phpstan needs the by-ref cache typed: array<int, string|null>.
        $schemaSlugById = [];
        ...
        foreach (($candidateResult['results'] ?? []) as $candidate) {
            ...
            if (isset($schemaSlugById[$schemaId]) === false) {
                try {
                    ...
                    $this->logger?->warning(
                        'WOO-536: schema slug lookup failed',
    ...
    private function listListedPublishedCatalogs(object $catalogiService): array
    {
        try {
            ...
            if ($catalogRegister === '' || $catalogSchema === '') {
                $this->logger?->warning(
                    'WOO-536: catalog_register/catalog_schema app-config not set — /api/search default scope is empty',
