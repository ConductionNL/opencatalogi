...
# publication-attachment-defaults Specification

## Purpose
Define how the publication-attachment upload dialog seeds its "Automatically
publish" toggle from per-schema configuration (`configuration.defaultAutoShare`),
so publication types can opt into a default-on toggle without changing the
per-upload override behaviour. Implemented in `src/modals/generic/UploadFiles.vue`.
