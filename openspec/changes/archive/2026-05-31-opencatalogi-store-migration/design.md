...
## Plugin availability (verified against `@conduction/nextcloud-vue@^1.0.0-beta.66`, the version pinned on `development`)
...
This change touches only `src/store/modules/object.js`, `jest.config.js`, and the new test mock. No `package.json` dependency change is required — the rebuild sits on `development`'s existing `@conduction/nextcloud-vue@^1.0.0-beta.66` pin, so there is no version-line merge conflict to resolve.
