...
nothing else: no IP addresses, user agents, cookies, sessions, or any personal
data are ever stored. Storage, RBAC, and querying are OpenRegister
(counter objects in a `usageCounter` schema — no bespoke tables, hydra
ADR-022); operational totals extend the existing `prometheus-metrics` endpoint
