...
test.describe('app-packaging (repository invariants)', () => {
	test(// @e2e app-packaging::licence-is-consistent-across-all-metadata-files
	'PKG-001 — info.xml, LICENSE, composer.json and publiccode.yml all declare EUPL-1.2', async () => {
	...
	test(// @e2e app-packaging::no-elasticsearch-claim
	'PKG-003 — the README claims no ElasticSearch backend and names OpenRegister SOLR', async () => {
	...
	test(// @e2e app-packaging::document-content-search-is-marked-planned
	'PKG-003 — document-content search is described as planned, not as shipped', async () => {
		const readme = read('README.md')
		const line = readme
			.split('\n')
	...
	test(// @e2e app-packaging::documented-path-is-reachable
	'PKG-004 — the aggregated-publications doc names the routed endpoint and entry point', async () => {
		...
		expect(
			doc,
			'the doc must cross-reference the federation capability',
		).toContain('FED-001')
...
test.describe('notifications (repository invariants)', () => {
	...
	test(// @e2e notifications::publication-carries-its-retention-notifications
	'NTF — the publication retention rules are the declarative dialect, with no imperative dispatch', async () => {
		const rules = registerSchemas().publication['x-openregister-notifications']
		expect(
			rules,
			'publication must carry its retention notification rules',
		).toBeTruthy()
		for (const key of ['retention-expiring-soon', 'retention-review-required']) {
			...
			expect(
				rule.trigger?.type,
				`${key} must declare a trigger type`,
			).toBeTruthy()
			...
			expect(
				rule.subject?.nl,
				`${key} must carry a Dutch subject`,
			).toBeTruthy()
			expect(
				rule.subject?.en,
				`${key} must carry an English subject`,
			).toBeTruthy()
		}
		...
		const walk = (dir: string): void => {
			for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
				...
				if (entry.isDirectory()) {
					...
					if (
						/OCP\\Notification|\bINotification\b|->createNotification\(/.test(
							body,
						)
					) {
		...
		expect(
			offenders,
			'notifications must stay declarative — no dispatch code in lib/',
		).toEqual([])
	})
})
...
test.describe('ooapi-catalog-publication (repository invariants)', () => {
	test(// @e2e ooapi-catalog-publication::opencatalogi-contains-no-raw-or-storage-sql-for-ooapi-resources
	'OOAPI — the OOAPI code reads through OpenRegister, never raw SQL or its own storage', async () => {
		...
		expect(
			ooapiFiles.length,
			'the OOAPI source files must be found',
		...
		for (const rel of ooapiFiles) {
			...
			for (const rx of forbidden) {
				expect(
					src,
					`${rel} must not use ${rx} — OOAPI reads through OpenRegister`,
				).not.toMatch(rx)
			}
		}
	})
