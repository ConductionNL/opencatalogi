 ...
 */
class WOO572RepairReadRules implements IRepairStep
{
    /**
     * Constructor.
     *
    ...
    /**
     * Whether OpenRegister is installed/enabled on this instance.
     *
     * `IAppManager::isEnabledForAnyone()` only exists on Nextcloud >= 32, while
     * the 1.x stable line this hotfix targets declares NC 28-34 in info.xml.
     * Fall back to `isInstalled()` (deprecated since 32 but still present up to
     * 35) so the repair step never fatals on an older core during `occ upgrade`.
     *
     * @return bool True when OpenRegister is available.
     */
    private function isOpenRegisterAvailable(): bool
    {
        if (method_exists($this->appManager, 'isEnabledForAnyone') === true) {
            return $this->appManager->isEnabledForAnyone('openregister') === true;
        }

        return $this->appManager->isInstalled('openregister') === true;
    ...
    {
        return "Backfill publication + document read-rule two-rule shape (WOO-572, port of WOO-536)";
    ...
    public function run(IOutput $output): void
    {
        ...
        if ($this->isOpenRegisterAvailable() === false) {
            $output->warning('OpenRegister app is not enabled - skipping WOO-572 read-rule backfill');
            return;
        }
        ...
        try {
            ...
            $output->warning('OpenRegister SchemaMapper unavailable - skipping WOO-572 read-rule backfill');
            return;
        }
        ...
        $output->info(
            sprintf(
                'WOO-572 read-rule backfill: %d schema(s) upgraded, %d skipped (already correct or customised)',
                $updated,
                $skipped
            )
        );
    ...
    private function maybeUpgradeSchema(object $schemaMapper, string $slug, IOutput $output): string
    {
        try {
            ...
            $output->warning("WOO-572: cannot list schemas by slug '{$slug}' — {$e->getMessage()}");
            return 'skipped';
        }

        if (empty($schemas) === true) {
            $output->info("WOO-572: schema '{$slug}' not present in this installation — nothing to upgrade");
            return 'skipped';
        }
        ...
        foreach ($schemas as $schema) {
            ...
            if (is_array($authorization) === false || isset($authorization['read']) === false) {
                $output->info(
                    "WOO-572: schema '{$slug}' (id ".$schema->getId().") has no authorization.read block — skipping"
                );
                continue;
            }
            ...
            try {
                ...
                $output->info(
                    "WOO-572: schema '{$slug}' (id ".$schema->getId().") upgraded to two-rule shape"
                );
                $updated = true;
                $this->logger->info(
                    'WOO-572 repair (WOO-536 port): schema authorization.read upgraded to two-rule shape',
                    ['schema' => $slug, 'schemaId' => $schema->getId()]
                );
            } catch (\Throwable $e) {
                $output->warning(
                    "WOO-572: failed to update schema '{$slug}' (id ".$schema->getId()."): {$e->getMessage()}"
                );
            }
        }//end foreach
