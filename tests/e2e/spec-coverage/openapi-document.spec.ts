...
test.describe('openapi-document', () => {
	test(// @e2e api-documentation::anonymous-client-fetches-the-document-cross-origin
	'API-DOC-003 — GET /api/openapi.json is anonymously reachable, returns JSON with CORS headers and the installed version', async ({
		...
		const res = await anon.get('/index.php/apps/opencatalogi/api/openapi.json', {
		...
		const versionResp = await authedRequest.get(
			'/index.php/apps/opencatalogi/api/settings/version',
		)
		if (versionResp.ok()) {
			const versionBody = await versionResp.json().catch(() => null)
			const installedVersion = (versionBody?.version
				?? versionBody?.appVersion) as string | undefined
			if (installedVersion) {
