...
// We'll count this properly later if needed
    ...
    public function attachments(string $id): JSONResponse
    {
        ...
        if (!$isPublished) {
            ...
            $registerAllowed = is_numeric($context['registers']) ? $object['@self']['register'] == $context['registers'] : (is_array($context['registers']) && in_array($object['@self']['register'], $context['registers']));

            $schemaAllowed = is_numeric($context['schemas']) ? $object['@self']['schema'] == $context['schemas'] : (is_array($context['schemas']) && in_array($object['@self']['schema'], $context['schemas']));
