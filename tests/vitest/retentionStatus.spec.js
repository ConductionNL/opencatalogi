...
describe('isExpiringSoon', () => {
	it('flags an object expiring within the window', () => {
		expect(isExpiringSoon({ retentionExpiresAt: daysFromNow(20) }, 30)).toBe(
			true,
		)
	})
	it('does not flag one expiring beyond the window', () => {
		expect(isExpiringSoon({ retentionExpiresAt: daysFromNow(40) }, 30)).toBe(
			false,
		)
	})
	it('does not flag an already-expired object as expiring-soon', () => {
		expect(isExpiringSoon({ retentionExpiresAt: daysFromNow(-1) }, 30)).toBe(
			false,
		)
	})
	it('never flags an archived object', () => {
		expect(
			isExpiringSoon(
				{ status: 'archived', retentionExpiresAt: daysFromNow(10) },
				30,
			),
		).toBe(false)
	})
...
describe('getRetentionStatus', () => {
	it('prioritises archived, then review-required, then expiring-soon', () => {
		...
		expect(getRetentionStatus({ retentionExpiresAt: daysFromNow(10) }, 30)).toBe(
			'expiring-soon',
		)
		expect(getRetentionStatus({ retentionExpiresAt: daysFromNow(90) }, 30)).toBe(
			'ok',
		)
	})
})
